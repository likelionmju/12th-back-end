...
public class Lotto {
	public Lotto() {}
	private float purchase_amount=0; // 구매 금액
	private int real_Num[] = new int[7]; // 당첨 번호
	private int issue_cnt=0; // 발행 매수
	private int[][] lotto_Num;  // 발행 번호 배열
	private float rate_of_profit = 0; // 수익률
	private float total_profit=0; // 총수익
	public float get_purchase_amount() {
		return purchase_amount;
	}
	
	public void set_purchase_amount(float amount) {
		purchase_amount = amount;
		issue_cnt = (int)purchase_amount / 1000;
		lotto_Num = new int[issue_cnt][6];
	}
	
	public void issue_lotto() {
		for(int i=0;i<issue_cnt;i++) {
			while(!get_RandomNumbers(lotto_Num[i])) { // 발행 번호 만들기 메소드
				;
			}
			
			for(int j=0;j<6;j++) {
				System.out.print(lotto_Num[i][j]+" "); // 발행번호 6개 출력
			}
			System.out.println();
		}
		System.out.println("총 발행 매수: "+issue_cnt+"장");
	}
	...
	public void check_JackPotNumber() {
		...
		for(int i=0;i<issue_cnt;i++) { // 발행 매수
			for(int j=0;j<6;j++) { // 발행 번호 6개 비교
				for(int k=0;k<7;k++) { // 당첨 번호 7개 비교
					if(lotto_Num[i][j] == real_Num[k]) {
	...
	public void calculate_profit_rate() {
		if(total_profit==0) {
			...
			rate_of_profit = (total_profit / purchase_amount*100);
